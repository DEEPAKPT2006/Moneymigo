...
export class AIService {
  ...
  async analyzeScenario(financialData: FinancialData, scenario: string): Promise<string> {
    return this.callAIForScenario(financialData, scenario);
  }
  ...
  private async callAIForScenario(financialData: FinancialData, scenario: string): Promise<string> {
    const apiKey = this.getApiKey();
    ...
    return this.callAIWithRetryForScenario(financialData, scenario, 3);
  }
  ...
  private async callAIWithRetryForScenario(financialData: FinancialData, scenario: string, maxRetries: number): Promise<string> {
    const apiKey = this.getApiKey();
    let lastError: Error | null = null;
    
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        const prompt = this.generateScenarioPrompt(financialData, scenario);
        ...
        if (!response.ok) {
          ...
          if (response.status === 503 || (errorInfo.error && errorInfo.error.code === 503)) {
            if (attempt < maxRetries - 1) {
              ...
              // Return fallback content instead of throwing error
              return this.getScenarioFallbackResponse(scenario);
            }
          }
    ...
    // All attempts failed - return fallback instead of throwing
    return this.getScenarioFallbackResponse(scenario, lastError);
  }
...
${isQuotaError ? '📅 Quota resets daily - check back tomorrow!' : isOverloadedError ? '⏳ High demand period - service will resume shortly.' : isApiKeyError ? '🔑 Configure API key in Settings.' : '⚡ Connecting to AI service...'}`;
  ...
  private generateScenarioPrompt(data: FinancialData, scenario: string): string {
    const { transactions, budgets, goals } = data;
...
- Total Expenses: ₹${totalExpenses.toLocaleString()}
- Net Balance: ₹${netBalance.toLocaleString()}
- Monthly Average Income: ₹${monthlyIncome.toLocaleString()}
- Monthly Average Expenses: ₹${monthlyExpenses.toLocaleString()}
- Monthly Average Savings: ₹${monthlySavings.toLocaleString()}
- Total Transactions: ${transactions.length}
- Data Period: ${monthCount} months

Current Goals:
${goals.map(g => 
  `- ${g.title}: ₹${g.currentAmount.toLocaleString()} / ₹${g.targetAmount.toLocaleString()} (${((g.currentAmount/g.targetAmount)*100).toFixed(1)}% complete)`
).join('\n')}

Current Budgets:
${budgets.map(b => 
  `- ${b.category}: ₹${b.spent.toLocaleString()} / ₹${b.limit.toLocaleString()} (${((b.spent/b.limit)*100).toFixed(1)}% used)`
).join('\n')}

Please provide a comprehensive analysis including:
1. Direct impact of this scenario on monthly finances
2. Specific numerical calculations (EMI, interest, savings impact, etc.)
3. Timeline implications (how long to achieve goals, payoff periods)
4. Risk assessment and recommendations
5. Alternative strategies if applicable

Use Indian Rupee (₹) formatting and provide specific, actionable insights. Keep the tone professional but approachable.
`;
  }

  private getScenarioFallbackResponse(scenario: string, error?: Error): string {
    const isApiKeyError = error?.message.includes('API key') || error?.message.includes('Invalid request');
    
    if (isApiKeyError) {
      return `📊 **Scenario Analysis Requires AI Configuration**

To analyze the scenario: "${scenario}"

🔑 **Setup Required:**
Please configure your Gemini API key in Settings to enable AI-powered scenario analysis.

💡 **Manual Analysis Tips:**
• Consider the monthly impact on your budget
• Calculate EMI payments for loans using online calculators
• Review your current savings rate vs new commitments
• Plan for contingencies with emergency funds

⚙️ Configure AI in Settings for detailed scenario modeling.`;
    }

    return `📊 **Scenario Analysis: "${scenario}"**

⚡ **Basic Impact Assessment:**
Your scenario involves significant financial planning. Here's a general framework:

💰 **Key Considerations:**
• Calculate monthly payment obligations
• Assess impact on current savings rate
• Review timeline for existing goals
• Consider emergency fund requirements

📈 **Next Steps:**
• Use financial calculators for precise numbers
• Review your current budget flexibility
• Consider multiple scenarios
• Plan for risk mitigation

🔄 Try again for AI-powered detailed analysis when service is available.`;
  }
