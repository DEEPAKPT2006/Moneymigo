...
export const DataProvider: React.FC<DataProviderProps> = ({ children }) => {
  ...
  const addTransaction = async (transactionData: Omit<Transaction, 'id' | 'createdAt'>) => {
    ...
    try {
      ...
      } else if (error.message === 'FIRESTORE_RULES_ERROR' || error.code === 'permission-denied') {
        setPermissionError(true);
        toast.error('Permission denied', {
          description: 'Firestore security rules need to be configured'
        });
      } else {
  ...
  // Show permission error guide if Firestore rules are blocking access
