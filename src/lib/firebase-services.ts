...
export const addBudget = async (userId: string, budgetData: Omit<Budget, 'id'>) => {
  ...
  try {
    ...
    return await handleFirestoreError(error, 'adding budget', performAdd);
  }
};

export const getUserBudgets = async (userId: string): Promise<Budget[]> => {
  if (!isFirestoreEnabled()) {
    console.warn('⚠️ Firestore not available, returning empty budgets');
    return [];
  }
  ...
  try {
    ...
    if (error.code === 'permission-denied') {
      console.warn('⚠️ Firestore permission denied - returning empty budgets');
      setPermissionError(true);
      return [];
    }
    await handleFirestoreError(error, 'fetching budgets');
    return []; // Return empty array as fallback
  }
};
...
export const addGoal = async (userId: string, goalData: Omit<Goal, 'id'>) => {
  ...
  try {
    ...
    return await handleFirestoreError(error, 'adding goal', performAdd);
  }
};

export const getUserGoals = async (userId: string): Promise<Goal[]> => {
  if (!isFirestoreEnabled()) {
    console.warn('⚠️ Firestore not available, returning empty goals');
    return [];
  }
  ...
  try {
    ...
    if (error.code === 'permission-denied') {
      console.warn('⚠️ Firestore permission denied - returning empty goals');
      setPermissionError(true);
      return [];
    }
    await handleFirestoreError(error, 'fetching goals');
    return []; // Return empty array as fallback
  }
};
...
export const subscribeToUserBudgets = (userId: string, callback: (budgets: Budget[]) => void) => {
  ...
  return onSnapshot(q, 
    ...
    (error) => {
      ...
      if (error.code === 'permission-denied') {
        console.warn('⚠️ Firestore permission denied - budgets subscription cancelled');
        setPermissionError(true);
        callback([]);
        return;
      }
      ...
      if (error.code === 'unavailable') {
        console.warn('⚠️ Firestore budgets subscription offline, will retry automatically');
      } else {
...
export const subscribeToUserGoals = (userId: string, callback: (goals: Goal[]) => void) => {
  ...
  return onSnapshot(q, 
    ...
    (error) => {
      ...
      if (error.code === 'permission-denied') {
        console.warn('⚠️ Firestore permission denied - goals subscription cancelled');
        setPermissionError(true);
        callback([]);
        return;
      }
      ...
      if (error.code === 'unavailable') {
        console.warn('⚠️ Firestore goals subscription offline, will retry automatically');
      } else {
        console.error('❌ Firestore goals subscription failed:', error);
      }
      callback([]);
    }
  );
